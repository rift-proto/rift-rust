...
///
/// Produced by [`StorageEngine`](crate::storage::StorageEngine) operations,
/// covering I/O failures, key-not-found, serialization, capacity, and
/// permission errors for both sync (Memory, Sled) and async (Redis) backends.
#[derive(Debug, Error)]
pub enum StorageError {
    /// An I/O failure occurred in the underlying storage engine.
    ///
    /// Carries a human-readable description. For backends that expose
    /// structured errors (e.g. `sled::Error`, `redis::RedisError`), the
    /// original error is available through the source chain.
    ...
    ///
    /// Typically indicates CBOR or JSON encoding issues when reading or
    /// writing values to the engine.
    #[error("storage serialization failure: {0}")]
    Serialization(String),

    /// An error from the underlying engine implementation.
    ///
    /// This variant wraps engine-specific errors (e.g. `sled::Error`,
    /// Redis errors) via the source chain. Use [`StorageError::engine`]
    /// to construct one from any `std::error::Error`.
    #[error("storage engine error: {0}")]
    Engine(#[source] BoxedStdError),

    /// The storage engine has exceeded its configured capacity.
...
impl StorageError {
    /// Wrap an arbitrary engine-level error into [`StorageError::Engine`],
    /// preserving it as the error source.
    ///
    /// This is the preferred constructor for backends that expose their own
    /// error types (e.g. `sled::Error`, `redis::RedisError`).
    pub fn engine<E: std::error::Error + Send + Sync + 'static>(e: E) -> Self {
...
///
/// ```rust,no_run
/// use rifts_core::error::{RiftError, TopicReject, Result};
///
...
///
/// This is a type alias for `Box<dyn std::error::Error + Send + Sync>`,
/// supporting any heap-allocated error type.
pub type BoxedStdError = Box<dyn std::error::Error + Send + Sync>;
