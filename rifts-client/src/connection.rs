...
use rifts_core::frame::{EncodingFormat, Frame, FrameType};
use rifts_core::frame_codec::{DEFAULT_MAX_BINARY_PAYLOAD, decode_binary_frame, encode_frame};
use rifts_core::message::SubscribeIntent;
use rifts_core::message::command::Reply;
use rifts_core::protocol::hello::{Ready, Welcome};
        ...
        Some("welcome") => {
            let w = Welcome {
                ...
                negotiated_codec: match obj.get("negotiated_codec").and_then(|v| v.as_str()) {
                    Some("cbor") | Some("Cbor") => EncodingFormat::Cbor,
                    _ => EncodingFormat::Json,
                },
            ...
            WsMessage::Text(text) => {
                match serde_json::from_str::<JsonValue>(&text) {
                    ...
                    Err(_) => {
                        ...
                        match rifts_core::frame_codec::decode_text_frame(text.as_bytes()) {
                            Ok(frame) => {
                                dispatch_frame(
                                    frame,
                                    &event_tx,
...
async fn dispatch_text_envelope(
    ...
    match msg_type {
        ...
        _ => {
            ...
            if let Ok(frame) = rifts_core::frame_codec::decode_text_frame(
        ...
        FrameType::Data | FrameType::Control => {
                ...
                && let Ok(obj) = serde_json::from_slice::<JsonValue>(payload)
            {
                ...
                match msg_class {
                    Some("event") => {
                        if let Ok(event) =
                            serde_json::from_value::<rifts_core::message::event::Event>(obj)
                        {
                    ...
                    Some("state") => {
                        if let Ok(state) =
                            serde_json::from_value::<rifts_core::message::state::State>(obj)
                        {
                    ...
                    Some("datagram") => {
                        if let Ok(datagram) =
                            serde_json::from_value::<rifts_core::message::datagram::Datagram>(obj)
                        {
                    ...
                    Some("stream") => {
                        if let Ok(segment) = serde_json::from_value::<
                            rifts_core::message::stream::StreamSegment,
                        >(obj)
                        {
                    ...
                    Some("snapshot") => {
                        if let Ok(snapshot) =
                            serde_json::from_value::<rifts_core::message::snapshot::Snapshot>(obj)
                        {
